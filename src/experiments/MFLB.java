...
public class MFLB {
	...
	public static void constantCAdjusting(String paraFilename, int paraNumUsers, int paraNumItems,
		...
		for (int i = 0; i < paraNumExperiments; i++) {
			for (int j = 0; j < tempNumC; j++) {
				...
				tempTestAucDeviationArray[j] += tempDifference * tempDifference
						/ paraNumExperiments;
			} // Of for j
		} // Of for i
		...
		for (int j = 0; j < tempNumC; j++) {
			...
			System.out.println("Test AUC: " + tempTestAverageAucArray[j] + " +- "
					+ Math.sqrt(tempTestAucDeviationArray[j]));
		} // Of for j
	...
	public static void main(String args[]) {
		try {
			constantCAdjusting("D:/data/movielens-943u1682m.txt", 943, 1682, 100000, 1, 5, 6000,
					10);
